...
class ParquetWriteSuite extends AnyFunSuite {
  ...
  private val data = Seq[User](User(1L, "a", 1.0, Array("a", "b"), Map("t1" -> "t1"), new Timestamp(mills)),
	User(2L, "b", 2.0, Array("b", "c"), Map("t2" -> "t2"), new Timestamp(mills)))
  ...
  test("write parquet file") {
	...
	spark.range(0, 10000000).toDF("id").map(row => {
	  val id = row.getLong(0)
	  val mills = Calendar.getInstance().getTimeInMillis
	  val user = User(id, Array.fill(id.toInt % 50)("a").mkString(","), id % 1000, Array.fill(50)(id.toString),
		Map("id" -> id.toString), new Timestamp(mills))
	  user
	}).write.mode(SaveMode.Overwrite).save(path)
  }
