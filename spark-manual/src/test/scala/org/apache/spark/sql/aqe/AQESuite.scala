...
class AQESuite extends SparkFunSuite {
  ...
  test("smj") {
	...
	val input4 = spark.sparkContext.parallelize((1 to 200).map(x => (x, x.toString)), 5).toDF("key", "value")
	val partitions1 = input4.rdd.partitions.length
	println(s"input4 partitions: $partitions1")
	val input5 = spark.sparkContext.parallelize(Seq(
	  (1, "1"), (1, "2"),
	  (2, "1"), (2, "2"),
	  (3, "1"), (3, "2")
	), 3).toDF("key", "value")
	val partitions2 = input5.rdd.partitions.length
	println(s"input5 partitions: $partitions2")
	
	val result = input4.as("t1").join(input5.as("t2"),
	  input4("key") === input5("key"), "leftouter").orderBy(input4("key").asc)
	
	result.select("t1.key", "t1.value", "t2.key", "t2.value").show()
