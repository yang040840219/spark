...
class MultiplicationNode(left: AlgebraNode, right: AlgebraNode)
    ...
    override def generateCode(): String = {
        val gen = (left.getType(), right.getType()) match {
            case (DataType.INT, DataType.DOUBLE) =>
                s"(${left.generateCode().toInt} * (double)${right.generateCode()})"
            case (DataType.INT, DataType.INT) =>
                s"(${left.generateCode()} * ${right.generateCode()})"
            case (DataType.DOUBLE, DataType.INT) =>
                s"((double)${left.generateCode()} * ${right.generateCode()})"
