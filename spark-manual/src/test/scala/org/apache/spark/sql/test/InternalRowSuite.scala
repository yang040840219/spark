...
class InternalRowSuite extends SparkFunSuite {
    ...
    test("implicit to tuple") {
        val spark = SparkSession.builder().master("local[2]").getOrCreate()
        import spark.implicits._

        // class 对象转换为 tuple
        type MyObjEncoded = (Int)

        implicit def toEncoded(o: MyObj): MyObjEncoded = (o.i)

        implicit def fromEncoded(e: MyObjEncoded): MyObj = new MyObj(e)

        val d = spark.createDataset(Seq[MyObjEncoded](
            new MyObj(1), new MyObj(2), new MyObj(3)
        )).toDF("i")
